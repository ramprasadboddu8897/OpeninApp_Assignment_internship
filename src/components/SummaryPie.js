import React from "react";
import PieChart from "./PieChart";

const SummaryPie = () => {
  return (
    <div className="summary mt-5 mb-5 h-100% w-full row-span-2 grid grid-rows-1 grid-cols-2 gap-8">
      <div className="pie--chart relative box-border bg-white rounded-2xl grid grid-rows-1 grid-cols-1">
        <h1 className="text-2xl absolute top-1 left-1 text-left px-8 py-4 font-semi-bold">
          Top products
        </h1>
        <select
          id="cars"
          name="cars"
          className="absolute top-4 right-4 w-fit px-4 py-2 opacity-70 self-end rounded-lg font-medium text-gray-600"
        >
          <option value="volvo">May-June 2022</option>
          <option value="saab">May-June 2023</option>
          <option value="fiat">May-June 2024</option>
          <option value="audi">May-June 2025</option>
        </select>
        <PieChart />
      </div>
      <div className="more--details relative bg-white rounded-2xl flex-col">
        <h1 className="text-2xl text-left px-8 py-4 font-semibold">
          Today's Schedule
        </h1>
        <div className="event--wrapper flex flex-col justify-around px-8">
          <div className="item border-l-4 border-[#9BDD7C] px-4 my-2">
            <h1 className="title text-lg font-semibold text-left my-0">
              Meeting with suppliers from Kuta Bali
            </h1>
            <p className="time text-left text-lg opacity-60">14:00 - 15:00</p>
            <p className="location text-left">at Sunset road ,kuta ,bali</p>
          </div>
          <div className="item border-l-4 border-[#6972C3] px-4 my-2">
            <h1 className="title text-lg font-semibold text-left my-0">
              Chack operation at Giga,factory 1
            </h1>
            <p className="time text-left text-lg opacity-60">18:00-19:00</p>
            <p className="location text-left">at central jakartha</p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default SummaryPie;
